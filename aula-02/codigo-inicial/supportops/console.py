...
class UsageSummary:
    """Aggregated usage for all model calls made during one agent run."""
...
def _reasoning_tokens_from_logprobs(message: AIMessage) -> int | None:
    """Count tokens whose decoded prefix exactly matches the reasoning trace.

    Ollama's ``eval_count`` combines thinking, answer, and special tokens. When
    logprobs are enabled, the decoded entries begin with the visible thinking
    trace, so this gives a useful trace-token count without pretending it is a
    provider-supplied usage breakdown.
    """
...
def _usage_panel(summary: UsageSummary) -> Panel:
    ...
    if summary.reasoning_source in {"trace_logprobs", "mixed", "partial"}:
        notes.append(
            Text(
                "* Contagem do texto de raciocínio via logprobs. "
                "O Ollama informa apenas o total de saída; tokens especiais podem "
                "fazer o eval_count ser ligeiramente maior.",
                style="dim",
            )
        )
...
def render_agent_result(
    ...
    *,
    ...
    """Render an agent result as a readable execution trace."""
    ...
    header = Group(
        Text("SupportOps Agent", style="bold cyan"),
        ...
        if isinstance(message, AIMessage):
            ...
            if message.tool_calls:
                ...
                for tool_call in message.tool_calls:
                    ...
                    table.add_row(name, _json_renderable(args))
                console.print(
                    Panel(
                        table,
            ...
            if content and message is not final_message:
                console.print(
                    Panel(Markdown(content), title="Resposta intermediária")
                )
            continue
    ...
    if final_message is not None:
        final_content = _content_text(final_message.content).strip()
        console.print(
            Panel(
