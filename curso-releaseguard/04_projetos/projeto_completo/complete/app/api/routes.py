...
from observability.metrics import DEPENDENCY
...
def checkout(req:CheckoutRequest):
    with tracer.start_as_current_span('checkout.create') as span:
        span.set_attribute('cart.id', req.cart_id)
        return _checkout_impl(req)

def _checkout_impl(req:CheckoutRequest):
...
def payment(req:PaymentRequest):
    with tracer.start_as_current_span('payment.request') as span:
        ...
        return _payment_impl(req)

def _payment_impl(req:PaymentRequest):
    ...
    if state.scenario=='payment_latency':
        with DEPENDENCY.labels('payment_provider').time(): time.sleep(0.25)
    if state.scenario=='payment_timeout':
        with DEPENDENCY.labels('payment_provider').time(): time.sleep(0.5)
        raise HTTPException(504,'payment provider timeout')
...
@router.post('/qa/execute-plan')
def execute_test_plan(plan: TestPlan):
    report=run_plan(plan, base_url='http://127.0.0.1:8000')
